...
export default {
     ...
     *
     * Sends a POST request with `_method=PUT` to handle file uploads.
     *
     * @param {number} id
     * @param {Object} blog
     * @param {string} blog.title
    ...
    async updateBlog(id, blog) {
        try {
            ...
            if (Array.isArray(blog.category_id)) {
                blog.category_id.forEach(id => formData.append('categories_id[]', String(id)));
            }
